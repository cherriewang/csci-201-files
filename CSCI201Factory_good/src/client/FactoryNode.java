	...
	public void draw(Graphics g, Point mouseLocation) {
		...
		if(mFObject == null) {
			...
			if(path > 0) {
				...
				if(colorCode >= pathColors.length) colorCode = pathColors.length-1;
				g.setColor(pathColors[colorCode]);
				g.fillOval(renderBounds.x + renderBounds.width/4, //left side 25% in
							renderBounds.y + renderBounds.height/4, //top 25% in
							renderBounds.width/2, //take up half the grid
							renderBounds.height/2); //take up half the grid
			}
		} else {
